...
const SandwichEdit=()=>{
    ...
    const onSubmit=async e=>{
        e.preventDefault();
        await axios.put(`http://localhost:8009/sandwich/v1/update/${foodId}`,food);
        alert("Food Details Updated Successfully");
        navigate("/sandwich");
    }
    const loadFood=async () =>{
        const result=await axios.get(`http://localhost:8009/sandwich/v1/id/${foodId}`);
       setFood(result.data);
    };
    function order(){
