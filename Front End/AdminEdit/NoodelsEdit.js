...
const NoodelsEdit=()=>{
    ...
    const onSubmit=async e=>{
        e.preventDefault();
        await axios.put(`http://localhost:9002/noodels/v1/update/${foodId}`,food);
        alert("Food Details Updated Successfully");
        navigate("/noodels");
    }
    const loadFood=async () =>{
        const result=await axios.get(`http://localhost:9002/noodels/v1/id/${foodId}`);
       setFood(result.data);
    };
    function order(){
