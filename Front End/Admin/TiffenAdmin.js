...
const TiffenAdmin=()=>{
    ...
    const deleteFood=(foodId)=>{
        const urll=`http://localhost:9003/tiffen/v1/delete/${foodId}`;
        axios.delete(urll).then(()=>{
            alert("Food Details Deleted Successfully!!!");
            axios.get("http://localhost:9003/tiffen/v1/view").then( res => {           
   ...
   function order(){
    ...
    return(
        ...
        <div>
         ...
         <CardBody>
           <Col> <h1  id="caption1">Tiffen's</h1></Col>
           <Col lg="10" md="4"><Button id="create1" variant="outline-primary" size="lg"> <Link to="/addt">Create</Link></Button></Col>
            </CardBody>
            ...
            <center><table>
                <tr>
                 ...
                 {TiffenAdmin.map( food => 
                                <tr>
                                    <td>{food.foodId}</td>
                                    <td>{<img src="https://th.bing.com/th/id/OIP.OBp42aXmaezC7tNSf4z6WgHaF7?w=241&h=193&c=7&r=0&o=5&dpr=1.3&pid=1.7" className="image"/>}</td>
                                    <td>{food.foodTitle}</td>
                                    <td>{food.foodDiscription}</td>
                                    <td>₹{food.foodPrice}-</td>
                                    <td>{food.foodDiscount} % OFF</td>
                                    <td><Link to={`/editt/${food.foodId}`}>
