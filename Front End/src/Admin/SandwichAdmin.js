...
const SandwichAdmin=()=>{
    ...
    const deleteFood=(foodId)=>{
        const urll=`http://localhost:8009/sandwich/v1/delete/${foodId}`;
        axios.delete(urll).then(()=>{
            alert("Food Details Deleted Successfully!!!");
            axios.get("http://localhost:8009/sandwich/v1/view").then( res => {           
                setFoodCategory(res.data);            
            })
        })
    }
   
    const [SandwichAdmin,setFoodCategory] = useState([]);
    const url = 'http://localhost:8009/sandwich/v1/view';
   ...
   function order(){
    ...
    return(
        ...
        <div>
         ...
         <CardBody>
           <Col> <h1  id="caption1">SandWich</h1></Col>
           <Col lg="10" md="4"><Button id="create1" variant="outline-primary" size="lg"> <Link to="/adds">Create</Link></Button></Col>
            </CardBody>
            ...
            <center><table>
                <tr>
                 ...
                 {SandwichAdmin.map( food => 
                                <tr>
                                    <td>{food.foodId}</td>
                                    <td>{<img src="https://th.bing.com/th/id/OIP.k4JMO5bD97BK3hxEHefrmAHaFP?w=220&h=180&c=7&r=0&o=5&dpr=1.3&pid=1.7" className="image"/>}</td>
                                    <td>{food.foodTitle}</td>
                                    <td>{food.foodDiscription}</td>
                                    <td>₹{food.foodPrice}-</td>
                                    <td>{food.foodDiscount} % OFF</td>
                                    <td><Link to={`/edits/${food.foodId}`}>
