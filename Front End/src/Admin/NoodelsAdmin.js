...
const NoodelsAdmin=()=>{
    ...
    const deleteFood=(foodId)=>{
        const urll=`http://localhost:9002/noodels/v1/delete/${foodId}`;
        axios.delete(urll).then(()=>{
            alert("Food Details Deleted Successfully!!!");
            axios.get("http://localhost:9002/noodels/v1/view").then( res => {           
                setFoodCategory(res.data);            
            })
        })
    }
   
    const [NoodelsAdmin,setFoodCategory] = useState([]);
    const url = 'http://localhost:9002/noodels/v1/view';
   ...
   function order(){
    ...
    return(
        ...
        <div>
         ...
         <CardBody>
           <Col> <h1  id="caption1">Chainees Food</h1></Col>
           <Col lg="10" md="4"><Button id="create1" variant="outline-primary" size="lg"> <Link to="/addn">Create</Link></Button></Col>
            </CardBody>
            ...
            <center><table>
                <tr>
                 ...
                 {NoodelsAdmin.map( food => 
                                <tr>
                                    <td>{food.foodId}</td>
                                    <td>{<img src="https://th.bing.com/th/id/OIP.ZKZNwJ55lwbMLdCGE9gxbAHaFD?w=290&h=198&c=7&r=0&o=5&dpr=1.3&pid=1.7" className="image"/>}</td>
                                    <td>{food.foodTitle}</td>
                                    <td>{food.foodDiscription}</td>
                                    <td>₹{food.foodPrice}-</td>
                                    <td>{food.foodDiscount} % OFF</td>
                                    <td><Link to={`/editn/${food.foodId}`}>
