...
const PizzaAdmin=()=>{
    ...
    const deleteFood=(foodId)=>{
        const urll=`http://localhost:8007/pizza/v1/delete/${foodId}`;
        axios.delete(urll).then(()=>{
            alert("Food Details Deleted Successfully!!!");
            axios.get("http://localhost:8007/pizza/v1/view").then( res => {           
                setFoodCategory(res.data);            
            })
        })
    }
   
    const [PizzaAdmin,setFoodCategory] = useState([]);
   ...
   function order(){
    ...
    return(
        ...
        <div>
         ...
         <CardBody>
           <Col> <h1  id="caption1">Pizza-Hut</h1></Col>
           <Col lg="10" md="4"><Button id="create1" variant="outline-primary" size="lg"> <Link to="/addp">Create</Link></Button></Col>
            </CardBody>
            ...
            <center><table>
                <tr>
                 ...
                 {PizzaAdmin.map( food => 
                                <tr>
                                    <td>{food.foodId}</td>
                                    <td>{<img src="https://th.bing.com/th/id/OIP.AZqGRDHUSm2vdcKVa_3QSwHaEu?w=281&h=180&c=7&r=0&o=5&dpr=1.3&pid=1.7" className="image"/>}</td>
                                    <td>{food.foodTitle}</td>
                                    <td>{food.foodDiscription}</td>
                                    <td>₹{food.foodPrice}-</td>
                                    <td>{food.foodDiscount} % OFF</td>
                                    <td><Link to={`/editp/${food.foodId}`}>
