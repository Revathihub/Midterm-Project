...
const RiceAdmin=()=>{
    ...
    const deleteFood=(foodId)=>{
        const urll=`http://localhost:9001/rice/v1/delete/${foodId}`;
        axios.delete(urll).then(()=>{
            alert("Food Details Deleted Successfully!!!");
            axios.get("http://localhost:9001/rice/v1/view").then( res => {           
                setFoodCategory(res.data);            
            })
        })
    }
   
    const [RiceAdmin,setFoodCategory] = useState([]);
    const url = 'http://localhost:9001/rice/v1/view';
   ...
   function order(){
    ...
    return(
        ...
        <div>
         ...
         <CardBody>
           <Col> <h1  id="caption1">Meals</h1></Col>
           <Col lg="10" md="4"><Button id="create1" variant="outline-primary" size="lg"> <Link to="/addm">Create</Link></Button></Col>
            </CardBody>
            ...
            <center><table>
                <tr>
                 ...
                 {RiceAdmin.map( food => 
                                <tr>
                                    <td>{food.foodId}</td>
                                    <td>{<img src="https://th.bing.com/th/id/OIP.-fSETj1xnMyz1vSausjyOgHaEW?pid=ImgDet&w=207&h=121&c=7&dpr=1.3" className="image"/>}</td>
                                    <td>{food.foodTitle}</td>
                                    <td>{food.foodDiscription}</td>
                                    <td>₹{food.foodPrice}-</td>
                                    <td>{food.foodDiscount} % OFF</td>
                                    <td><Link to={`/editm/${food.foodId}`}>
