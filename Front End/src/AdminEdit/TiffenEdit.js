...
const TiffenEdit=()=>{
    ...
    const onSubmit=async e=>{
        e.preventDefault();
        await axios.put(`http://localhost:9003/tiffen/v1/update/${foodId}`,food);
        alert("Food Details Updated Successfully");
        navigate("/tiffen");
    }
    const loadFood=async () =>{
        const result=await axios.get(`http://localhost:9003/tiffen/v1/id/${foodId}`);
       setFood(result.data);
    };
    function order(){
